  ...
  describe("Write", () => {
    ...
    describe("Create", () => {
      it("should create the collection and document if it does not exist", async () => {
        ...
        const target = createTestTarget();
        // Act.
        const [$airbnb, $youtube] = await target.create(databaseCompanies, [
          airbnb,
          youtube,
        ]);
        // Assert.
      ...
      it("should return a rejected promise if there are duplicate ids in the given template set", async () => {
        ...
        const target = createTestTarget();
        // Act.
        const actual = target.create(databaseCompanies, [
          youtube,
          airbnb,
          youtube,
        ]);
    ...
    describe("Update", () => {
      it("should set the fields that matches the filter", async () => {
        ...
        [youtube, airbnb] = await target.create(databaseCompanies, [
          youtube,
          airbnb,
        ]);
        ...
        // Act.
        const actual = await target.update<IZBrand>(
          databaseCompanies,
          expected,
          filter,
        );
      ...
      it("should set the fields for all documents", async () => {
        ...
        // Act.
        const actual = await target.update<IZBrand>(databaseCompanies, {
          name: "",
        });
        // Assert.
        expect(actual).toEqual(2);
      ...
      it("should reject if an attempt is made to change the _id", async () => {
        ...
        const filter = new ZFilterBinaryBuilder()
          ...
          .build();
        // Act.
        const actual = target.update<IZDocumentWithId>(
          databaseCompanies,
          { _id: "not-allowed" },
          filter,
        );
