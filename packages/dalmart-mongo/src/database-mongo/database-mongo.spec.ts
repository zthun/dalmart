...
describe("ZDatabaseMongo", () => {
  ...
  describe("Connection", () => {
    function assertConnection<T>(
      expected: T,
      targetFn: () => ZDatabaseMongo,
      connectionFn: (t: ZDatabaseMongo) => T,
    ) {
      // Arrange
  ...
  describe("CRUD", () => {
    ...
    describe("Read", () => {
      ...
      it("sorts the data in ascending order.", async () => {
        // Arrange
        const target = await createPopulatedTarget();
        const expected = parents.sort((a, b) =>
        ...
        const actual = await target.read(
          parentsSource,
          new ZDataRequestBuilder().sort(sorted).build(),
        );
        // Assert
        expect(actual).toEqual(expected);
      });
      ...
      it("returns page data.", async () => {
        ...
        const actual = await target.read(
          parentsSource,
          new ZDataRequestBuilder().size(2).build(),
        );
        // Assert
      ...
      it("returns the correct page.", async () => {
        ...
        const actual = await target.read(
          parentsSource,
          new ZDataRequestBuilder().page(2).size(2).build(),
        );
        // Assert
      ...
      it("returns the empty array if retrieving any pages beyond the last.", async () => {
        ...
        const actual = await target.read(
          parentsSource,
          new ZDataRequestBuilder().page(2).size(parents.length).build(),
        );
        // Assert
        expect(actual).toEqual([]);
      });
      ...
      it("returns the remaining items on the last page.", async () => {
        ...
        const actual = await target.read(
          parentsSource,
          new ZDataRequestBuilder().page(2).size(3).build(),
        );
        // Assert
      ...
      describe("Filter", () => {
        const shouldReturnExpectedData = async (
          expected: any[],
          filter: IZFilter,
        ) => {
          // Arrange
        ...
        describe("Logic", () => {
          describe("And", () => {
            it("returns expected data", async () => {
              const a = new ZFilterBinaryBuilder()
                .subject("rating")
                .greaterThan()
                .value(3)
                .build();
    ...
    describe("Update", () => {
      ...
      it("updates all items that match the filter.", async () => {
        ...
        const target = await createPopulatedTarget();
        const expected = [
          Object.assign({}, fred, template),
          Object.assign({}, barney, template),
          wilma,
          betty,
        ];
        // Act
